...
{
	conf_path = L"C:\\data\\repos\\Sandbox\\Virtual-Display-Driver-Ref\\Virtual Display Driver (HDR)";
	std::wcout << L"Config Path is at default value: " + conf_path + L"\n";
...
void Refactoring::SettingsLoader::OverrideDefaultsRegistry()
{
	...
	reg_reader.GetSetting<bool>(L"logging", L"SendLogsThroughPipe",
								settings.logs.send_logs_through_pipe);
	...
	reg_reader.GetSetting<bool>(L"hdr10_static_metadata", L"enabled",
	...
	reg_reader.GetSetting<std::wstring>(
		L"bit_depth_management", L"force_bit_depth",
	...
	reg_reader.GetSetting<std::wstring>(
		L"color_space", L"primary_color_space",
