...
{
	xml_reader.GetSetting<bool>(keys({"logging", "logging"}), settings.logs.enable_standard_logs);
	xml_reader.GetSetting<bool>(keys({"logging", "debuglogging"}), settings.logs.enable_debug_logs);
	xml_reader.GetSetting<bool>(keys({"logging", "SendLogsThroughPipe"}), settings.logs.send_logs_through_pipe);

	xml_reader.GetSetting<bool>(keys({"edid", "CustomEdid"}), settings.edid.custom_edid);
	xml_reader.GetSetting<bool>(keys({"edid", "PreventSpoof"}), settings.edid.prevent_manufacturer_spoof);
	xml_reader.GetSetting<bool>(keys({"edid", "EdidCeaOverride"}), settings.edid.edid_cea_override);

	// colour
	xml_reader.GetSetting<bool>(keys({"colour", "HDRPlus"}), settings.colours.hdr_plus);
	xml_reader.GetSetting<bool>(keys({"colour", "SDR10bit"}), settings.colours.sdr10);
	// g_settings.colors.HDR_COLOR = g_settings.colors.hdr_plus ? IDDCX_BITS_PER_COMPONENT_12 :
	// IDDCX_BITS_PER_COMPONENT_10; g_settings.colors.SDR_COLOR = g_settings.colors.sdr10 ? IDDCX_BITS_PER_COMPONENT_10
	// : IDDCX_BITS_PER_COMPONENT_8;
	xml_reader.GetSetting<std::string>(keys({"colour", "ColourFormat"}), settings.colours.color_format);

	// Cursor
	xml_reader.GetSetting<bool>(keys({"cursor", "HardwareCursor"}), settings.cursor.hardware_cursor);
	xml_reader.GetSetting<bool>(keys({"cursor", "AlphaCursorSupport"}), settings.cursor.alpha_cursor_support);
	xml_reader.GetSetting<int>(keys({"cursor", "CursorMaxX"}), settings.cursor.max_x);
	xml_reader.GetSetting<int>(keys({"cursor", "CursorMaxY"}), settings.cursor.max_y);
	...
	// === LOAD NEW EDID INTEGRATION SETTINGS ===
	xml_reader.GetSetting<std::string>(keys({"edid_integration", "edid_profile_path"}),
									   settings.edid_integration.profile_path);
	xml_reader.GetSetting<bool>(keys({"edid_integration", "enabled"}), settings.edid_integration.enabled);
	xml_reader.GetSetting<bool>(keys({"edid_integration", "auto_configure_from_edid"}),
								settings.edid_integration.auto_configure);
	xml_reader.GetSetting<bool>(keys({"edid_integration", "override_manual_settings"}),
								settings.edid_integration.override_manual_settings);
	xml_reader.GetSetting<bool>(keys({"edid_integration", "fallback_on_error"}),
								settings.edid_integration.fallback_on_error);

	// === LOAD HDR ADVANCED SETTINGS ===
	xml_reader.GetSetting<bool>(keys({"hdr_advanced", "hdr10_static_metadata", "enabled"}),
								settings.hdr_advanced.static_metadata_enabled);
	xml_reader.GetSetting<double>(keys({"hdr_advanced", "hdr10_static_metadata", "max_display_mastering_luminance"}),
								  settings.hdr_advanced.max_display_mastering_luminance);
	xml_reader.GetSetting<double>(keys({"hdr_advanced", "hdr10_static_metadata", "min_display_mastering_luminance"}),
								  settings.hdr_advanced.min_display_mastering_luminance);
	xml_reader.GetSetting<int>(keys({"hdr_advanced", "hdr10_static_metadata", "max_content_light_level"}),
							   settings.hdr_advanced.max_content_light_level);
	xml_reader.GetSetting<int>(keys({"hdr_advanced", "hdr10_static_metadata", "max_frame_avg_light_level"}),
							   settings.hdr_advanced.max_frame_avg_light_level);

	// === LOAD AUTO RESOLUTIONS SETTINGS ===
	xml_reader.GetSetting<std::string>(keys({"auto_resolutions", "source_priority"}),
									   settings.auto_resolutions.source_priority);
	...
	xml_reader.GetSetting<std::string>(keys({"hdr_advanced", "color_space", "primary_color_space"}),
									   settings.hdr_advanced.color_space.primary_color_space);
	xml_reader.GetSetting<bool>(keys({"hdr_advanced", "color_space", "enabled"}),
								settings.hdr_advanced.color_space.enabled);
	xml_reader.GetSetting<double>(keys({"hdr_advanced", "color_space", "gamma_correction"}),
								  settings.hdr_advanced.color_space.gamma_correction);
	xml_reader.GetSetting<bool>(keys({"hdr_advanced", "color_space", "enable_matrix_transform"}),
								settings.hdr_advanced.color_space.enable_matrix_transform);
	...
	// xml_reader.GetSetting<bool>(keys({"not_implemented", "not_implemented"}),
	//							settings.monitor_emulation.manufacturer_emulation_enabled);
	// xml_reader.GetSetting<bool>(keys({"not_implemented", "not_implemented"}), settings.monitor_emulation.enabled);
	// xml_reader.GetSetting<bool>(keys({"not_implemented", "not_implemented"}),
	//							settings.monitor_emulation.emulate_physical_dimensions);
