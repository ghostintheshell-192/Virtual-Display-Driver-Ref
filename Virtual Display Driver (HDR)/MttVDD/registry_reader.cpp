...
bool Refactoring::RegistryReader::OpenRegistry()
{
	...
	LONG lResult =
		RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\MikeTheTech\\VirtualDisplayDriver", 0, KEY_READ, &reg_handle_key);
...
void Refactoring::RegistryReader::InitializePath(std::string &path) const
{
	...
	lResult = RegGetValueA(reg_handle_key, "", "VDDPATH", RRF_RT_REG_SZ, NULL, NULL, &dwBufferSize);
	...
	lResult = RegGetValueA(reg_handle_key, "", "VDDPATH", RRF_RT_REG_SZ, NULL, (LPBYTE)&path[0], &dwBufferSize);
	if (lResult == ERROR_SUCCESS)
	{
...
std::string Refactoring::RegistryReader::GetRawRegistryValue(HKEY hKey, const std::string &setting)
{
	...
	CharUpperBuffA(reg_name.data(), static_cast<DWORD>(reg_name.size()));

	DWORD type = 0;
	DWORD buffer_size = 0;

	LONG lResult = RegGetValueA(hKey, "", reg_name.c_str(), 0, &type, NULL, &buffer_size);
	if (lResult != ERROR_SUCCESS)
		return "";
	...
	if (type == REG_DWORD)
	{
		...
		lResult = RegGetValueA(hKey, "", reg_name.c_str(), 0, NULL, (LPBYTE)&value, &buffer_size);
		if (lResult != ERROR_SUCCESS)
			return "";
	...
	else if (type == REG_SZ)
	{
		...
		lResult = RegGetValueA(hKey, "", reg_name.c_str(), 0, NULL, (LPBYTE)&value[0], &buffer_size);
		if (lResult != ERROR_SUCCESS)
			return "";
		return value;
	}
