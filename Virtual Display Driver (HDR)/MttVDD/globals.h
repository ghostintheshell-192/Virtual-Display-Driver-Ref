...
struct MonitorProfile
{
	...
	ColorMatrix Get_sRGB()
	{
		...
		t_matrix.matrix[1][1] = static_cast<float>(gamma_correction) / 2.2f; // Green
	...
	ColorMatrix Get_DCI_P3()
	{
		...
		t_matrix.matrix[1][1] = 1.0419f * (static_cast<float>(gamma_correction) / 2.4f);
	...
	ColorMatrix Get_REC_2020()
	{
		...
		t_matrix.matrix[1][1] = 1.1316f * (static_cast<float>(gamma_correction) / 2.4f);
		t_matrix.matrix[1][2] = 0.0f;
	...
	ColorMatrix Get_Adobe_RGB()
	{
		...
		t_matrix.matrix[1][1] = 1.0f * (static_cast<float>(gamma_correction) / 2.2f);
		t_matrix.matrix[2][2] = 1.0f * (static_cast<float>(gamma_correction) / 2.2f);
		return t_matrix;
	};
};
